...
const { toggleFollow, toggleLike, addComment, getComments, getFeed, addFriendByCode,getFriends ,getFriendProfile,toggleFistBump,addPrivateComment,getNotifications,markNotificationRead,getLogDetail} = require('../controllers/socialController');
...
router.post('/comment', authMiddleware, addPrivateComment); // API gửi lời nhắn động viên riêng tư (Day 40)
router.get('/notifications', authMiddleware, getNotifications); // API lấy thông báo (Day 40)
router.put('/notifications/:notifId/read', authMiddleware, markNotificationRead); // API đánh dấu thông báo đã đọc (Day 40)
router.get('/logs/:logId', authMiddleware, getLogDetail); // API lấy chi tiết một Log
