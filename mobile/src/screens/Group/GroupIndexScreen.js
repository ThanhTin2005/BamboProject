...
export default function GroupIndexScreen({ navigation }) {
    ...
    // ⚡ TÁCH HÀM FETCH RA ĐỂ TÁI SỬ DỤNG KHI REFRESH
    const fetchGroups = async (isActive = true) => {
        try {
    ...
    // ⚡ THÊM HÀM XÓA/GIẢI TÁN NHÓM
    const confirmDeleteGroup = (groupId) => {
        Alert.alert(
            "Cảnh báo nguy hiểm",
            "Việc giải tán nhóm sẽ xóa vĩnh viễn toàn bộ dữ liệu, thành viên và minh chứng bên trong. Chắc chắn giải tán?",
            [
                { text: "Hủy", style: "cancel" },
                {
                    text: "Giải tán nhóm",
                    ...
                    onPress: async () => {
                        try {
                            const token = await AsyncStorage.getItem('userToken');
                            // Gọi API Xóa nhóm (Backend ông nhớ check quyền Leader như đã thống nhất nhé)
                            ...
                            // Backend trả về lỗi 403 nếu không phải Leader
                            const errorMsg = error.response?.data?.message || "Không thể xóa nhóm lúc này. Bạn có phải trưởng nhóm không?";
    ...
    const renderGroupItem = ({ item }) => (
        <TouchableOpacity 
          ...
          // ⚡ THÊM SỰ KIỆN NHẤN GIỮ HIỆN MENU TÙY CHỌN
          onLongPress={() => {
              const options = [
                  { text: "Hủy", style: "cancel" },
                  { text: "Chỉnh sửa nhóm", onPress: () => navigation.navigate('EditGroup', { groupId: item.group_id, group: item }) }
              ];

              // Luôn hiện nút Giải tán, Backend sẽ là người chặn nếu không phải Leader. 
              // (Hoặc nếu API my-groups của ông có trả về `item.role`, ông có thể bọc nó trong `if (item.role === 'leader')`)
              options.push({
                  text: "Giải tán nhóm",
                  style: "destructive",
                  onPress: () => confirmDeleteGroup(item.group_id)
              });

              Alert.alert(
                  "Tùy chọn nhóm",
                  `Bạn muốn làm gì với nhóm "${item.title}"?`,
                  options,
                  { cancelable: true }
              );
          }}
...
const styles = StyleSheet.create({
    ...
    goalRow: { flexDirection: 'row', backgroundColor: '#fff', marginHorizontal: 0, marginBottom: 16, borderRadius: 20, overflow: 'hidden', elevation: 3, shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 8, alignItems: 'stretch', minHeight: 120 },
    rowImage: { width: 120, borderTopLeftRadius: 20, borderBottomLeftRadius: 20, resizeMode: 'cover', backgroundColor: '#F0F5F0' },
